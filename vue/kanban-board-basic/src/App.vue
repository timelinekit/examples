...
import { ref } from 'vue';
import { KanbanBoard, type KanbanBoardRef, setLicense } from '@timelinekit/vue';
...
const boardRef = ref<KanbanBoardRef>();
...
function handleReady() {
  ...
  board.events.cardClick$.subscribe((args) => {
...
function onAddCard() {
  ...
  if (!board) return;
  const cols = [];
  for (let i = 0; i < board.data.columnsLength; i++) {
    cols.push(board.data.getColumn(i));
  }
...
<template>
  <div class="app">
    ...
    <div class="board-container">
      <KanbanBoard ref="boardRef" @ready="handleReady" />
    </div>
  </div>
</template>
