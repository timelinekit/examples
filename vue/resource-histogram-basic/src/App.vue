...
import { ref } from 'vue';
import { ResourceHistogram, type ResourceHistogramRef, setLicense } from '@timelinekit/vue';
...
const histogramRef = ref<ResourceHistogramRef>();

function handleReady() {
  const histogram = histogramRef.value;
  if (!histogram) return;
...
<template>
  <div class="app">
    <h1>TimelineKit - Resource Histogram</h1>
    <div class="toolbar">
      <button @click="histogramRef?.zoomIn()">Zoom In</button>
      <button @click="histogramRef?.zoomOut()">Zoom Out</button>
      <button @click="histogramRef?.zoomToFit()">Zoom to Fit</button>
    </div>
    <div class="histogram-container">
      <ResourceHistogram ref="histogramRef" @ready="handleReady" />
    </div>
  </div>
</template>
